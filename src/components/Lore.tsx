...
const inventory: InventoryItem[] = [
    { name: "Unity Engine", iconClass: "bi-unity", description: "Primary development environment" },
    ...
    return (
        <section id="lore" className="py-5 section-padding position-relative">
            <div className="container py-lg-5">
                <h2 className="display-4 text-center mb-5 text-uppercase tracking-widest fw-bold text-body animate-fade-in-up mobile-header" style={{ fontSize: 'calc(1.5rem + 1.5vw)' }}>Character Sheet</h2>

                <div className="row g-4 g-lg-5">
                    {/* Bio / Backstory */}
                    <div className="col-12 mb-2 animate-fade-in-up" style={{ animationDelay: '0.1s' }}>
                    ...
                    {/* Skill Tree */}
                    <div className="col-12 col-lg-6 animate-fade-in-up" style={{ animationDelay: '0.2s' }}>
                    ...
                    {/* Inventory */}
                    <div className="col-12 col-lg-6 animate-fade-in-up" style={{ animationDelay: '0.3s' }}>
                        <div className="glass rounded-4 p-4 p-md-5 shadow-sm h-100">
                            <h3 className="h4 fw-bold mb-4 text-body border-bottom border-secondary border-opacity-25 pb-3">
                            ...
                            <div className="row g-2 g-sm-3 mt-2">
                                {inventory.map((item, index) => (
                                    <div className="col-4 col-sm-4" key={index}>
                                        <div
                                            className="p-2 p-sm-3 text-center border border-secondary border-opacity-25 rounded-3 hover-scale hover-opacity cursor-pointer h-100 d-flex flex-column justify-content-center align-items-center bg-body bg-opacity-10"
                                            title={item.description}
                                            onClick={onOpenSecret}
                                        >
                                            <i className={`bi ${item.iconClass} fs-2 fs-sm-1 text-body mb-2`}></i>
                                            <span className="fw-medium text-body xsmall-text d-block">{item.name}</span>
                                        </div>
                                    </div>
                                ))}
                            </div>
                ...
                <div className="row mt-5 animate-fade-in-up" style={{ animationDelay: '0.4s' }}>
                    <div className="col-12">
                        <div className="glass rounded-4 p-4 p-md-5 shadow-sm">
                            <div className="d-flex flex-column flex-md-row justify-content-between align-items-md-center mb-5 border-bottom border-secondary border-opacity-25 pb-3 gap-3">
                                <h3 className="h4 fw-bold text-body m-0">
                                    <i className="bi bi-map me-2"></i>The Arsenal (Quest Log)
                                </h3>
                                <a href={`${import.meta.env.BASE_URL}resume.pdf`} target="_blank" rel="noopener noreferrer" className="btn btn-outline-primary rounded-pill px-4 shadow-sm hover-scale d-flex align-items-center justify-content-center gap-2">
                            ...
                            <div className="timeline position-relative ps-4 border-start border-primary border-2">
                                {[
                                    { year: '2021', title: 'Footwear Design', role: 'Footwear Designer', description: 'Designed footwear for various university projects and collab brands.' },
                                    ...
                                    { year: '2025', title: 'Game Design Journey', role: 'Game Design Intern', description: 'Learned the basic mechanics of professional game development.' },
                                ].map((item, idx) => (
                                    <div key={idx} className="timeline-item mb-5 position-relative">
                                        ...
                                        <div className="d-flex flex-column flex-sm-row align-items-start align-items-sm-center gap-2 gap-sm-3 mb-2">
                                            <span className="badge bg-body bg-opacity-25 text-body p-2 fs-6">{item.year}</span>
                                            <h4 className="h5 fw-bold m-0 text-body">{item.title}</h4>
                                        </div>
                                        <h5 className="h6 fw-medium text-body opacity-75">{item.role}</h5>
                                        <p className="text-body opacity-75 mb-0 small">{item.description}</p>
                                    </div>
                                ))}
                            </div>
                        </div>
